...
public class GuiSelectedPlayer {
    ...
    public void open(Player player, UUID selectedPlayer, String playerName) {
        GuiConfig guiConfig = me.sk8ingduck.friendsystemgui.FriendSystemGUI.getInstance().getSettingsConfig();
        ...
        MySQL mysql = me.sk8ingduck.friendsystemgui.FriendSystemGUI.getInstance().getMysql();
        mysql.arePlayersFriends(player.getUniqueId(), selectedPlayer, areFriends -> {
            ...
            Bukkit.getScheduler().scheduleSyncDelayedTask(me.sk8ingduck.friendsystemgui.FriendSystemGUI.getInstance(), () -> gui.open(player));
        });
    }
