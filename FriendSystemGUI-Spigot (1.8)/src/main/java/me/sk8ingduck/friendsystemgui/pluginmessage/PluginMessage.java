	...
	public void onPluginMessageReceived(String channel, Player player, byte[] message) {
		...
		if (subchannel.equals("friendsList")) {
			...
			for (int i = 0; i < size; i++) {
				friends.add(new Friend(UUID.fromString(in.readUTF()), in.readUTF(), in.readBoolean(), in.readUTF(), in.readUTF()));
			}
			...
			playerInfoCallbacks.get(uuid).onReceive(in.readBoolean(), in.readBoolean(), in.readBoolean());
