...
public class MySQL {
	...
	public MySQL(String host, int port, String username, String password, String database) {
		...
		try (Connection connection = dataSource.getConnection()) {
			...
			if (!tables.next()) {
				stmt.executeUpdate("ALTER TABLE request ADD COLUMN requestDate DATETIME DEFAULT NOW();");
				stmt.executeUpdate("UPDATE request SET requestDate=NOW();");
			}

			stmt.close();
		} catch (SQLException e) {
			FriendSystem.server.getConsoleCommandSource()
					.sendMessage(Component.text("[FriendSystem] MySQL Connection could not be established. Error:"));
			e.printStackTrace();
		}
	}
