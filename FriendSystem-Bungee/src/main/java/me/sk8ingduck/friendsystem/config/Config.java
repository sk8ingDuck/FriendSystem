...
public class Config {
	...
	public Config(String name, String path, boolean copyDefault) {
		...
		if (!file.exists() || (copyDefault && !containsComments(file))) {
			try {
				if (file.exists()) {
					Files.move(file.toPath(), new File(path, "old_" + name).toPath(), StandardCopyOption.REPLACE_EXISTING);
				}
				...
				if (!copyDefault) {
					...
					ProxyServer.getInstance().getLogger().info("§c[FriendSystem] Recreated config file " + name + " with default values because of plugin update!");
					ProxyServer.getInstance().getLogger().info("§c[FriendSystem] Old config file was saved as old_" + name);
				}
	...
	public Object getPath(String path, boolean translateColors) {
		if (fileConfiguration.get(path) == null) {
			...
			ProxyServer.getInstance().getLogger().info("§c[FriendSystem] Recreated config file " + name + " because Path '" + path + "' was null!");
			ProxyServer.getInstance().getLogger().info("§c[FriendSystem] Old config file was saved as old_" + name);
		}
	...
	private void recreateFromResources() {
		try {
			if (file.exists()) {
				Files.move(file.toPath(), new File(file.getParent(), "old_" + file.getName()).toPath(), StandardCopyOption.REPLACE_EXISTING);
			}
